"""
Steam《Cyberpunk 2077》评论采集脚本（可直接在 PyCharm 点击 Run）

首次运行前，在 PyCharm Terminal 执行：
    pip install requests

采样规则：最新英文评论；推荐/不推荐、Steam购买/非Steam购买均保留。
目标为20,000条唯一评论；若Steam没有更多数据，则保存已取得的最大数量。
"""
...
FILE_PREFIX = "cyberpunk_2077_reviews_20000"
CSV_PATH = OUTPUT_DIR / f"{FILE_PREFIX}_full_fields.csv"
STATE_PATH = OUTPUT_DIR / f"{FILE_PREFIX}_state.json"
METADATA_PATH = OUTPUT_DIR / f"{FILE_PREFIX}_metadata.json"
QUALITY_REPORT_PATH = OUTPUT_DIR / f"{FILE_PREFIX}_quality_report.json"
...
def fetch_page(session: requests.Session, cursor: str) -> dict[str, Any]:
    params = {
        "json": 1,
        "filter": FILTER,
        "language": LANGUAGE,
    ...
    for attempt in range(MAX_RETRIES + 1):
        try:
            ...
            if data.get("success") != 1:
                raise RuntimeError(
                    f"Steam API返回success={data.get('success')!r}"
                )
            return data
            ...
            print(
                f"请求失败：{error}；{wait_seconds:.1f}秒后重试。",
                flush=True,
            )
...
def normalize_review(review_data: dict[str, Any], scraped_at: str) -> dict[str, Any]:
    ...
    return {
        ...
        # 使用空白字符计数；正式NLP阶段可再使用更严格的tokenizer。
...
def write_reports(
    ...
    metadata = {
        ...
        "sampling_method": "Consecutive most-recent Steam reviews using cursor pagination",
        "api_parameters": {
            "filter": FILTER,
            "language": LANGUAGE,
            "review_type": "all",
        ...
        "fields": FIELDS,
        "playtime_unit": "hours",
        "timestamp_created_updated_last_played_unit": "Unix seconds UTC",
        "scraped_at_utc_format": "ISO 8601 UTC",
    ...
    quality_report = {
        ...
        "language_counts": dict(
            Counter(str(row.get("language", "")) for row in rows)
        ),
    ...
    try:
        while len(seen_ids) < TARGET_REVIEWS:
            ...
            if should_save:
                ...
                print(
                    f"已请求{pages_requested:,}页；已保存"
                    f"{min(len(seen_ids), TARGET_REVIEWS):,}/"
