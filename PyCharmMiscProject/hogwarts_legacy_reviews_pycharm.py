"""
Steam《Hogwarts Legacy》评论采集脚本（可直接在 PyCharm 点击 Run）

首次运行前，在 PyCharm Terminal 执行：
    pip install requests

脚本采集最新英文评论，保留推荐/不推荐及不同购买来源，目标为
20,000 条唯一评论。若 Steam 没有更多数据，则保存已取得的最大数量。
"""
...
FILE_PREFIX = "hogwarts_legacy_reviews_20000"
    ...
    for attempt in range(MAX_RETRIES + 1):
        try:
            ...
            if data.get("success") != 1:
                raise RuntimeError(
                    f"Steam API返回 success={data.get('success')!r}"
                )
            return data
...
def normalize_review(review_data: dict[str, Any], scraped_at: str) -> dict[str, Any]:
    ...
    return {
        ...
        # 该字段只在部分Steam Deck评论中出现，缺失时保留为空白。
